...
JLoader::register('JmailalertsModelEmails', JPATH_SITE . '/components/com_jmailalerts/models/emails.php');
$model = new JmailalertsModelEmails();
...
for ($s = 0; $s < count($qryConcat); $s++)
{
	...
	JLoader::register('JmailalertsTableAlert', JPATH_ROOT . '/administrator/components/com_jmailalerts/tables/alert.php');
    $getAlertDetails = new JmailalertsTableAlert(Factory::getDbo());
    $getAlertDetails->load(array('id' => $altid[$s]));
    $decodedGroups = json_decode($getAlertDetails->usergroup, true);
	$userGroup = is_array($decodedGroups) ? array_map('intval', $decodedGroups) : [];
	$currentUserGroup = Factory::getUser()->groups;
	$dpeAdmin = Factory::getUser()->authorise('core.manageall', 'com_cluster');
	$guest = Factory::getUser()->guest;
	$matchedUserGroup = array_intersect($currentUserGroup, (array)$userGroup);
	$guestUserGroup = array_intersect((array)$guest, (array)$userGroup);

	if((!$dpeAdmin && !$matchedUserGroup) && (!$guestUserGroup))
	{	
		continue; 
	}
...
<div class="mb-3 card">
	<div class="card-header">
		<div class="form-check">
			<input type="checkbox" name="alt[]" id="alert_' . $altid[$s] . '"
				...
				class="form-check-input alertname" />
	...
	<div class="card-body "style="visibility: hidden; height: 0; overflow: hidden;">
