...
// Language loading moved to plugin constructor to avoid initialization issues
...
try {
	...
	// TJQueue component not available, skip loading
}
...
class PlgSystemDpe extends CMSPlugin
{
	...
	{	
		...
		$app->redirect(Uri::root().'index.php?option=com_users&view=profile&layout=edit&Itemid=' . $itemId, false);die("Dpe Hack");
	}
	...
	public function onAfterMultiagencySave($id, $data)
	{
		...
		// Validate lead_consultant_id before using it
		$leadConsultantId = !empty($data['lead_consultant_id']) ? (int) $data['lead_consultant_id'] : 0;
		
		if ($leadConsultantId > 0)
		{
			$leadConsultantsGroups   = Factory::getUser($leadConsultantId)->groups;
			$leadConsultantUser      = Factory::getUser($leadConsultantId);
		}
		else
		{
			$leadConsultantsGroups   = array();
			$leadConsultantUser      = null;
		}
		...
		$xrfId                        = $table->id;
		$currentLeadConsultantId      = !empty($table->lead_consultant_id) ? (int) $table->lead_consultant_id : 0;
		
		if ($currentLeadConsultantId > 0)
		{
			$currentLeadConsultantsGroups = Factory::getUser($currentLeadConsultantId)->groups;
		}
		else
		{
			$currentLeadConsultantsGroups = array();
		}
		...
		if ((!$isNew) && (in_array($leadConsultantGroupId, $currentLeadConsultantsGroups)))
		{
			...
			{
				// Require_once $path;
				$componentPath = JPATH_SITE . '/components/com_multiagency';
				JLoader::register('MultiagencyFrontendHelpers', $componentPath . '/helpers/multiagency.php');
	...
	public function sendExternalCertificateEmails($recordDetails, $adminUserId)
	{
		...
		if (empty($adminUserId) || (int) $adminUserId <= 0)
		{
			return;
		}
		...
		if ($recordOwner && (int) $recordOwner > 0)
		{
...
public function onAfterRopDeleteRemoveRecordFromFlatTable($recordId)
{
	...
	public function onDocmanBeforeRequest($query)
    {
        ...
        if (isset($query->view) && $query->view === 'document' && isset($query->slug))
        {
			...
			// Disable ACL for lookup
			$model->setState([
				'access' => null
			]);

			$overrideTagTitle = trim($this->params->get('marketing_tags', ''));
			$document = $model
				->slug($query->slug)
   				->fetch();

            if (!$document->isNew()) {
                 // Check tag_list string
                 $tagList = explode(', ', $document->tag_list);

                 foreach($tagList as $tagTitle) {
                     if (strtolower(trim($tagTitle)) === $overrideTagTitle) {
                         $query->access = null;
