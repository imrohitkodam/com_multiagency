...
class DpeSlaActivitiesDatasource
{
	...
	public function getDataCountlinkboxTjdashcount()
	{
		...
		$link . $licenseFilter . '&filter[sla_status]=I&Itemid=' . $itemId, false
		);

		$urls[Text::_('PLG_TJDASHBOARDSOURCE_DPE_SLA_COMPLETE')] = Route::_(
		$link . $licenseFilter . '&filter[sla_status]=C&Itemid=' . $itemId, false
		);

		$urls[Text::_('PLG_TJDASHBOARDSOURCE_DPE_SLA_CANCEL')] = Route::_(
		$link . $licenseFilter . '&filter[sla_status]=CN&Itemid=' . $itemId, false
		);

		$items = [];
		$items['data'] = ['count' => $this->getData()
		,'titleLink'  => Route::_(
			$link . $licenseFilter . '&Itemid=' . $itemId, false
			)
		// ,'link'  => $urls
		];
