...
class DpemakingtheroundsDatasource
{
	public $dataSourceName = "PLG_TJDASHBOARDSOURCE_DPE_MAKINGROUNDS";
	...
	public function getData()
	{
		try
		{
			...
			$params = DPE::config();
			$requestStatus = (int) $params->get('makingtheRounds', '0');
			
			// Create a new query object.
			$query = $db->getQuery(true);

			// Select the required fields from the table.
			$query->select("count(a.id) AS ucmcount, SUM(CASE WHEN fcv.value = 'In progress' THEN 1 ELSE 0 END) as progresscount, SUM(CASE WHEN fcv.value = 'Action required' THEN 1 ELSE 0 END) as actionrequired, SUM(CASE WHEN fcv.value = 'DPO review' THEN 1 ELSE 0 END) as dporeview, SUM(CASE WHEN fcv.value = 'Complete' THEN 1 ELSE 0 END)
			as Completed");
			...
			$query->where($db->qn('a.client') . " = 'com_tjucm.monitoringcompliancemakingtherounds'");
		...
		$progresscount = ($makingrounds['progresscount']) ? $makingrounds['progresscount'] : 0;
		$actionrequired = ($makingrounds['actionrequired']) ? $makingrounds['actionrequired'] : 0;
		$dporeview = ($makingrounds['dporeview']) ? $makingrounds['dporeview'] : 0;
		$Completed     = ($makingrounds['Completed']) ? $makingrounds['Completed'] : 0;
		$ucmCount      = ($makingrounds['ucmcount']) ? $makingrounds['ucmcount'] : 0;

		$recordInfo = array();
		$recordInfo['widgetcolor'] = array(
		Text::_('PLG_TJDASHBOARDSOURCE_DPE__MAKINGROUNDS_INPROGRESS') => (($progresscount == 0) ? 'text-success' : 'text-amber'),
		Text::_('PLG_TJDASHBOARDSOURCE_DPE__MAKINGROUNDS_ACTIONREQ') => (($actionrequired == 0) ? 'text-red-500' : 'text-amber'),
		Text::_('PLG_TJDASHBOARDSOURCE_DPE__MAKINGROUNDS_DPOREV') => (($dporeview == 0) ? 'text-success' : 'text-amber'),
		// Text::_('PLG_TJDASHBOARDSOURCE_DPE_MAKINGROUNDS_COMPLETED') => (($Completed == 0) ? 'text-success' : 'text-amber')
		);

		$recordInfo['widgetdata'] = array(
		Text::_('PLG_TJDASHBOARDSOURCE_DPE__MAKINGROUNDS_INPROGRESS') => $progresscount,
		Text::_('PLG_TJDASHBOARDSOURCE_DPE__MAKINGROUNDS_ACTIONREQ') => $actionrequired,
		Text::_('PLG_TJDASHBOARDSOURCE_DPE__MAKINGROUNDS_DPOREV') => $dporeview,
		// Text::_('PLG_TJDASHBOARDSOURCE_DPE_MAKINGROUNDS_COMPLETED') => $Completed
		);

		$recordInfo['total']       = $ucmCount;
		$recordInfo['id']          = "makintherounds";
	...
	{
		$link = 'index.php?option=com_tjucm&view=items&client=com_tjucm.monitoringcompliancemakingtherounds&Itemid=1317';
