...
/**
 * DPE plugin for tjdashboardsource to get SAR log details
 *
 * @since  __DEPLOY_VERSION__
 */

class DpeSarLogDatasource
{
	public $dataSourceName = "PLG_TJDASHBOARDSOURCE_DPE_SARLOG";
	...
	public function getData()
	{
		try
		{
			...
			$query->select("count(a.id) AS ucmcount, SUM(CASE WHEN fcv.value = 'In progress' THEN 1 ELSE 0 END) as progresscount ,SUM(CASE WHEN fcv.value = 'Open' THEN 1 ELSE 0 END) as open ,
		...
		$ucmCount      = ($sarData['ucmcount']) ? $sarData['ucmcount'] : 0;
		 $openCount    = ($sarData['open']) ? $sarData['open'] : 0;

		$recordInfo = array();
		$recordInfo['widgetcolor'] = array(
		Text::_('PLG_TJDASHBOARDSOURCE_DPE_SARLOG_OPEN') => (($openCount == 0) ? 'text-success' : 'text-lightblue'),
		Text::_('PLG_TJDASHBOARDSOURCE_DPE_SARLOG_INPROGRESS') => (($progresscount == 0) ? 'text-success' : 'text-amber'),
		Text::sprintf('PLG_TJDASHBOARDSOURCE_DPE_SARLOG_DUE', self::SETDAYS) => (($dueDate == 0) ? 'text-success' : 'text-red-500'),
		Text::_('PLG_TJDASHBOARDSOURCE_DPE_SARLOG_OVERDUE') => (($overDate == 0) ? 'text-success' : 'text-red-500'),
		
		// Text::_('PLG_TJDASHBOARDSOURCE_DPE_SARLOG_CLOSED') => 'text-success'
		);

		$recordInfo['widgetdata'] = array(
		Text::_('PLG_TJDASHBOARDSOURCE_DPE_SARLOG_OPEN')	=>$openCount,
		Text::_('PLG_TJDASHBOARDSOURCE_DPE_SARLOG_INPROGRESS') => $progresscount,
	...
	public function getFormattedData()
	{
		...
		$urls = array(Text::_('PLG_TJDASHBOARDSOURCE_DPE_SARLOG_INPROGRESS') => '', Text::_('PLG_TJDASHBOARDSOURCE_DPE_SARLOG_CLOSED') => '');

		$urls[Text::_('PLG_TJDASHBOARDSOURCE_DPE_SARLOG_INPROGRESS')] = Route::_(
