...
class SubusersViewRoles extends HtmlView
{
	...
	protected function addToolbar()
	{
		...
		{
			if (isset($canDo->{'core.create'}) && $canDo->{'core.create'})
			{
			...
			if (isset($canDo->{'core.edit'}) && $canDo->{'core.edit'} && isset($this->items[0]))
			{
				ToolbarHelper::editList('role.edit', 'JTOOLBAR_EDIT');
			}
		}

		if (isset($canDo->{'core.edit.state'}) && $canDo->{'core.edit.state'})
		{
		...
		if (isset($this->items[0]->state))
		{
			if ($state->get('filter.state') == -2 && isset($canDo->{'core.delete'}) && $canDo->{'core.delete'})
			{
				ToolbarHelper::deleteList('', 'roles.delete', 'JTOOLBAR_EMPTY_TRASH');
				ToolbarHelper::divider();
			}
			elseif (isset($canDo->{'core.edit.state'}) && $canDo->{'core.edit.state'})
			{
		...
		if (isset($canDo->{'core.delete'}) && $canDo->{'core.delete'})
		{
			ToolbarHelper::deleteList('JGLOBAL_CONFIRM_DELETE', 'roles.delete', 'JTOOLBAR_DELETE');
			ToolbarHelper::divider();
		}

		if (isset($canDo->{'core.admin'}) && $canDo->{'core.admin'})
		{
