...
class MultiagencyModelUserForm extends FormModel
{
	...
	public function loadSubFormData($userId)
	{
		...
		{
			// Require_once $path;
			JLoader::register('MultiagencyFrontendHelpers', JPATH_ROOT . '/components/com_multiagency' . '/helpers/multiagency.php');
	...
	protected function loadFormData()
	{
		...
		if (!empty($data->id))
		{
			...
			if (ComponentHelper::getComponent('com_cluster', true)->enabled)
			{
				...
				$data->set('agency_role_map', $subFormArray);
			}
	...
	// Check login user associated with school - start
		...
		{
			Factory::getApplication()->enqueueMessage(Text::_('COM_MULTIAGENCY_STAFF_CREATION_SOMETHING_IS_WRONG'), 'error');
	...
	// Joomla user entry
		$formData = new Registry($app->getInput()->get('jform', '', 'array'));
	...
	public function validate($form, $data, $group = null)
	{
		...
		if ($data['id'])
		{
			...
			{
				Factory::getApplication()->enqueueMessage(Text::_('COM_MULTIAGENCY_EMAIL_MESSAGE'), 'error');
		...
		if (!$data['id'])
		{
			...
			if ($userexist)
			{
				Factory::getApplication()->enqueueMessage(Text::_('COM_MULTIAGENCY_ADD_USER_USER_ALREADY_EXIST'), 'error');
		...
		if ($data['id'])
		{
			...
			{
				Factory::getApplication()->enqueueMessage(Text::_('COM_MULTIAGENCY_FORM_RESET_PASSWORD_EMPTY'), 'error');
			...
			if ($data['random_password'] == 1)
			{
				...
				{
					Factory::getApplication()->enqueueMessage(Text::_('COM_MULTIAGENCY_FORM_ERROR_PASSWORD_EMPTY'), 'error');
				...
				{
					Factory::getApplication()->enqueueMessage(Text::_('COM_MULTIAGENCY_FORM_ERROR_CONFIRM_PASSWORD_EMPTY'), 'error');
				...
				{
					Factory::getApplication()->enqueueMessage(Text::_('COM_MULTIAGENCY_FORM_ERROR_PASSWORD_NOT_MATCH'), 'error');
		...
		*/
		...
		if (!count($roleList))
		{
			Factory::getApplication()->enqueueMessage(Text::_('COM_MULTIAGENCY_NO_ROLE_SELECTED'), 'error');
		...
		{
			Comment condition for staff for multiple organisations
			Factory::getApplication()->enqueueMessage(Text::_('COM_MULTIAGENCY_ALLOW_ADD_MEMEBER_ONLY_ONE'), 'error');
		...
		*/
		...
		{
			Factory::getApplication()->enqueueMessage(Text::_('COM_MULTIAGENCY_ADD_MEMEBER_DUPLICATION'), 'error');
