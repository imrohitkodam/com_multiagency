...
// 		$this->setRedirect(Route::_('index.php?option=com_multiagency&view=licenceform&layout=edit', false));
	...
	public function save($key = null, $urlVar = null)
	{
		...
		if ($return === false)
		{
			...
			// Redirect back to the edit screen.
			$id = (int) $app->getUserState('com_multiagency.edit.licence.id');
			$this->setMessage(Text::sprintf('Save failed', 'An error occurred'), 'warning');
			$this->setRedirect(Route::_('index.php?option=com_multiagency&view=licenceform&layout=edit&id=' . $id . '&Itemid=' . $itemId, false));
		}

		// Redirect back to the list screen.
		$this->setMessage(Text::sprintf('COM_LICENCE_ITEM_SAVED_SUCCESSFULLY', 'An error occurred'), 'Success');

		// DPE - Hack - Start -> Redirect to custom DPE school view
		$redirectUrl = $app->getInput()->getString('redirectUrl', '');
	...
	{
		$app = Factory::getApplication();
		$managerId = $app->getInput()->getInt('userId');
		$courseId = $app->getInput()->getInt('courseId');
	...
	{
		$app = Factory::getApplication();
		$licenceType = $app->getInput()->get('licenceType', '', 'STRING');
		$agencyId = $app->getInput()->getInt('agencyId');
		$courseId = $app->getInput()->getInt('courseId');
