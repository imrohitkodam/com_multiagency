...
class SubusersModelMapping extends AdminModel
{
	...
	public function getTable($type = 'Mapping', $prefix = 'SubusersTable', $config = array())
	{
		...
		return $mvcFactory->createTable($type, $prefix, $config);
	}
	...
	public function duplicate(&$pks)
	{
		...
		foreach ($pks as $pk)
		{
			...
			{
				throw new \RuntimeException('Table load failed for pk: ' . $pk);
			}
		}
    ...
    public function save($data)
    {
		...
		$data['checked_out'] = Factory::getApplication()->getIdentity()->id;
        $data['checked_out_time'] = (new Date('now'))->toSql();
        $data['ordering']= 0 ;
        $data['state']= 1 ;
