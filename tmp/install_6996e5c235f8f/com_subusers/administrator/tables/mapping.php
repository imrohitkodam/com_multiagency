...
class SubusersTablemapping extends Table
{
	 ...
	 * Constructor
	 *
	 * @param   \Joomla\Database\DatabaseInterface  &$db  A database connector object
	 */
	...
	public function bind($array, $ignore = '')
	{
		...
		if (($task == 'save' || $task == 'apply') && (!Factory::getApplication()->getIdentity()->authorise('core.edit.state', 'com_subusers') && $array['state'] == 1))
		{
			$array['state'] = 0;
		}
		...
		if (!Factory::getApplication()->getIdentity()->authorise('core.admin', 'com_subusers.mapping.' . $array['id']))
		{
	...
	{
		// We will retrieve the parent-asset from the Asset-table using MVCFactory
		$mvcFactory = Factory::getApplication()
			->bootComponent('com_subusers')
			->getMVCFactory();
		$assetParent = $mvcFactory->createTable('Asset', 'Administrator');
