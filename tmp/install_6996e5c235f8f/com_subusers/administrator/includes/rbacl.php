...
class RBACL
{
	...
	public static function table($name)
	{
		...
		// Use MVCFactory instead of Table::getInstance for Joomla 6 compatibility
		$mvcFactory = Factory::getApplication()
			->bootComponent('com_subusers')
			->getMVCFactory();

		// @TODO Add support for cache
		return $mvcFactory->createTable($name, 'SubusersTable');
	}
	...
	{
		// Use MVCFactory instead of BaseDatabaseModel::getInstance for Joomla 6 compatibility
		$mvcFactory = Factory::getApplication()
			->bootComponent('com_subusers')
			->getMVCFactory();

		// @TODO Add support for cache
		return $mvcFactory->createModel($name, 'Administrator', $config);
	}
	...
	public static function check($userId, $client, $action, $actionClient = null, $contentId = null)
	{
		...
		// If the requested user is different from the current user, load them
		if ($user->id != $userId)
		{
