 ...
 */
class JFormFieldAgency extends ListField
{
	/**
	 * The form field type.
	 *
	...
	protected function getOptions()
	{
		...
		$userId               = $app->getInput()->get('id', '', 'INT');
		...
		elseif ($app->getInput()->get('view') == 'submit' || $app->getInput()->get('view') == 'rsticketspro')
		{
			$agencies = $MultiagencyModel->getAllocatedAgencies($user->id);
		}
		else
		...
		if ($app->getInput()->get('view') == 'items' || ($app->getInput()->get('view') == 'rsticketspro' && count($agencies) > 1))
		{
		...
		elseif ($app->getInput()->get('view') == 'users' && $app->getInput()->get('layout') != 'import')
		{
