...
\Joomla\CMS\HTML\HTMLHelper::script('media/com_multiagency/js/multiagency.js' );

\Joomla\CMS\Language\Text::script('COM_MULTIAGENCY_ENROLMENT_SELECT_ENROLL_ITEMS');
...
?>
<form action="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_multiagency&view=enrollment&license='. $this->license); ?>" method="post" name="adminForm" id="adminForm">

	<div class="modal-content">
		<legend><?php echo \Joomla\CMS\Language\Text::sprintf('COM_MULTIAGENCY_ENROLMENT_AGENCY_FOR', $this->agencyData->title); ?><button type="button" class="close" onclick="closeAssignRecommendPopups();">&times;</button></legend>

	</div>
	<div class="filter-search btn-group"><?php
		if ($this->licenseType == 'all')
		{
			echo \Joomla\CMS\HTML\HTMLHelper::_('select.genericlist', $this->courseoptions, 'selectedcourse[]', 'class="btn input-medium" size="10" name="groupfilter"  	onchange="this.form.submit();"',"value", "text", $this->courseFilter);
		}
		?>
		</div>
...
<input type="text" name="filter_search" id="filter_search" class="span2" style="padding-top:0;padding-bottom:0"
		...
		if(empty($this->courseFilter) && $this->licenseType === 'all')
		{
			...
			<div class="alert alert-info"><?php echo \Joomla\CMS\Language\Text::_("COM_MULTIAGENCY_SELECT_COURSE");?></div>
			<?php
		}
		else
		{
		...
		<table class="table table-striped" id="multiagencyList">
			<thead>
				<tr>
					...
					<th class=''>
						<?php echo \Joomla\CMS\HTML\HTMLHelper::_( 'grid.sort', 'COM_MULTIAGENCY_ENROLMENT_USER_NAME', 'u.name', $listDirn, $listOrder); ?>
					</th>
					<th class=''>
						<?php echo \Joomla\CMS\HTML\HTMLHelper::_( 'grid.sort', 'COM_MULTIAGENCY_ENROLMENT_USER_USERNAME', 'u.email', $listDirn, $listOrder); ?>
					</th>
					<th class=''>
						<?php echo \Joomla\CMS\HTML\HTMLHelper::_( 'grid.sort', 'COM_MULTIAGENCY_ENROLMENT_ROLE', 'rolename', $listDirn, $listOrder); ?>
					</th>
					<th class=''>
						<?php echo \Joomla\CMS\HTML\HTMLHelper::_( 'grid.sort', 'COM_MULTIAGENCY_ENROLMENT_USERID', 'subusers.user_id', $listDirn, $listOrder); ?>
					</th>
				</tr>
			</thead>
<!--
...
-->
			<tfoot>
				<tr>
					<td colspan="<?php echo isset($this->items[0]) ? count(get_object_vars($this->items[0])) : 5; ?>">
						<a class="btn btn-primary enroll-btn pull-right" name="enroll" onclick="enroll();return false;" value=""><?php echo \Joomla\CMS\Language\Text::_('COM_MULTIAGENCY_ENROLMENT_BTN'); ?></a>
					</td>
				</tr>
			</tfoot>
			<tbody>
				...
				<td class="center">
							<?php echo \Joomla\CMS\HTML\HTMLHelper::_('grid.id', $i, $item->user_id, false, 'uid'); ?>
						</td>
