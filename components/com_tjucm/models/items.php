...
class TjucmModelItems extends ListModel
{
	...
	protected function populateState($ordering = "a.id", $direction = "DESC")
	{
		...
		// Set state for field filters - check if tjfields component exists
...
// DPE Hack
	...
	{
		// Check if required tjfields tables exist
		$db = $this->getDbo();
		$tables = $db->getTableList();
		$prefix = $db->getPrefix();
		$requiredTables = array(
			$prefix . 'tjfields_fields_value',
			$prefix . 'tjfields_fields_value_flat'
		);
		...
		// If tables don't exist, return empty query
		if (!$tablesExist)
		{
			$query = $db->getQuery(true);
			$query->select('NULL')->from($db->qn('#__tj_ucm_data'))->where('1=0');
			return $query;
		}
...
// echo $query->dump();
	...
	public function showListCheck($client)
	{
		if (!empty($client))
		{
			...
			// Check if tjfields_fields table exists
			try
			{
				$query = $db->getQuery(true);
			...
			{
				// Table doesn't exist (com_tjfields not installed)
				return false;
			}
		}
		else
